...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Get translations
  const noResultsText = window.HEXO_CONFIG
    ? window.HEXO_CONFIG.search_no_results || "No results found for"
    : "No results found for";
  const resultText = window.HEXO_CONFIG
    ? window.HEXO_CONFIG.search_result || "result"
    : "result";
  const resultsText = window.HEXO_CONFIG
    ? window.HEXO_CONFIG.search_results || "results"
    : "results";
  const loadingErrorText = window.HEXO_CONFIG
    ? window.HEXO_CONFIG.search_error || "Error loading search data"
  ...
  fetch(searchJsonPath)
    ...
    .then((data) => {
      console.log(
        "[VSC4T] Search index loaded successfully, entries:",
        data.length,
      );
      searchData = data;
      initSearch();
  ...
  function updateRecentSearches() {
    ...
    if (recentSearches.length === 0) {
      recentSearchesList.innerHTML =
        '<div class="recent-search-empty">No recent searches</div>';
      return;
    }
